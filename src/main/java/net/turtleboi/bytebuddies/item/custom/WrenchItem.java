...
public class WrenchItem extends Item {
    ...
    public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> pTooltipComponents, TooltipFlag pIsAdvanced) {
        ...
        if (nbtData != null && nbtData.contains(dockPos) && nbtData.contains(dockDimension)) {
            BlockPos blockPos = BlockPos.of(nbtData.getLong(dockPos));
            String dimensionString = nbtData.getString(dockDimension);
            pTooltipComponents.add(Component.literal("Linked Dock: " + blockPos.toShortString() + " (" + dimensionString + ")"));
            return;
        }

        pTooltipComponents.add(Component.literal("Linked Dock: <none>"));
    }

    private static void setDock(ItemStack wrenchStack, Level level, BlockPos blockPos) {
        CompoundTag nbtData = wrenchStack.getOrCreateTag();
    ...
    private static Optional<GlobalPos> getDock(ItemStack wrenchStack) {
        CompoundTag nbtData = wrenchStack.getTag();
        if (nbtData == null) return Optional.empty();
        ...
        BlockPos blockPos = BlockPos.of(nbtData.getLong(dockPos));
        ResourceLocation dimensionLocation = ResourceLocation.tryParse(nbtData.getString(dockDimension));
